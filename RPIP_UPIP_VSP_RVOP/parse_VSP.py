...
# version 1.0:2024.06.25--2024.07.10
import os,sys,re
...
with open(args.json, "r") as load_f:
    ...
    key_name=['totalRawReads','uniqueReadsProportion','postQualityReads','postQualityReadsProportion','removedInDehostingReadsProportion','libraryQScore']
    cloumn_name =['Total Raw Reads','% Unique Reads','Post-Quality Reads','% Post-Quality Reads','% Dehosted','library Q-Score']
    description=[
    ...
    key_name=['targetedMicrobial','untargeted','ambiguous','unclassified','lowComplexity','targetedInternalControl']
    cloumn_name+=['Targeted Microbial','Untargeted','Ambiguous','Unclassified','Low Complexity','Targeted Internal Control']
    for key in key_name:
    ...
    cloumn_name=['Microorganism','Present','Accessions','% Coverage','% ANI','Aligned Reads','Median Depth','RPKM','Absolute Quantity']
    for i in range(0, len(cloumn_name)):
        if i == 0:
            ...
            if 'consensusGenomeSequences' in species:###############output consensus sequences
                ...
                for key in species['consensusGenomeSequences']:#Consensus genome information. Included for RPIP viruses only.
                    ...
                    out4_file.write(">%s|reference:%s|description:%s|reference_length:%s|maximum_alignment_length:%s"
                                    ...
                                    "|median_depth:%s\n%s\n"
                                    %(species['name'],key['referenceAccession'],key['referenceDescription'],key['referenceLength'],
                                      key['maximumAlignmentLength'],key['maximumGapLength'],key['maximumUnalignedLength'],
                                      key['coverage'],key['ani'],key['alignedReadCount'],key['medianDepth'],key['sequence']))
                out4_file.close()
            ...
            if 'variants' in species:
                for variant in species['variants']:
                    ...
                    out3_file.write("%s\t%s\t.\t%s\t%s\t.\tPASS\tAF=%s;DP=%s;SG=%s"%(
                        variant['referenceAccession'],variant['referencePosition'],variant['referenceAllele'],variant['variantAllele'],
